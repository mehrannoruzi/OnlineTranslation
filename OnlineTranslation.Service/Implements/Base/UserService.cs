...
using OnlineTranslation.InfraStructure;
using OnlineTranslation.Service.Resource;
using OnlineTranslation.DataAccess.Dapper;
using DomainStrings = OnlineTranslation.Domain.Resource.Strings;
...
namespace OnlineTranslation.Service
{
    public class UserService : IUserService, IUserActionProvider
    {
        ...
        public async Task<IResponse<bool>> DeleteAsync(Guid userId)
        {
            ...
            var stores = _appUow.UserRepo.Get(conditions: x => x.UserId == userId, orderBy: null);
            if (stores != null)
                foreach (var store in stores) _appUow.UserRepo.Delete(store);
            var saveResult = await _appUow.ElkSaveChangesAsync();
